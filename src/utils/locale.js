...
// import getWeeksInMonth from 'date-fns-jalali/getWeeksInMonth';
...
getMonthComps(month, year) {
  ...
  getCalendarDays({ weeks, monthComps, prevMonthComps, nextMonthComps }) {
    ...
    const prevMonthDaysToShow =
      firstWeekday +
      (firstWeekday < firstDayOfWeek ? 7 : 0) -
      firstDayOfWeek;
    let prevMonth = true;
    let thisMonth = false;
    let nextMonth = false;
    ...
    if (localeId === "fa") {
      ...
      todayMonth = today.getMonth() + 1; // JavaScript months are 0-indexed
